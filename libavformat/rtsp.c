...
const AVOption ff_rtsp_options[] = {
    ...
    { "rtsp_transport", "set RTSP transport protocols", OFFSET(lower_transport_mask), AV_OPT_TYPE_FLAGS, {.i64 = 1 << RTSP_LOWER_TRANSPORT_TCP}, INT_MIN, INT_MAX, DEC|ENC, .unit = "rtsp_transport" }, \
...
redirect:
    ...
    if (rt->control_transport == RTSP_MODE_TUNNEL) {
        ...
        ff_log_event("RTSP_CONNECT_START", "\"host\":\"%s\",\"port\":%d", host, port);
        if ((ret = ffurl_open_whitelist(&rt->rtsp_hd, tcpname, AVIO_FLAG_READ_WRITE,
                       &s->interrupt_callback, &proto_opts, s->protocol_whitelist, s->protocol_blacklist, NULL)) < 0) {
            ff_log_event("RTSP_CONNECT_FAILED", "\"host\":\"%s\",\"port\":%d,\"error\":%d", host, port, ret);
            av_dict_free(&proto_opts);
            err = ret;
            goto fail;
        }
        ff_log_event("RTSP_CONNECT_SUCCESS", "\"host\":\"%s\",\"port\":%d", host, port);
        av_dict_free(&proto_opts);
